...
def should_chunk_chatterbox(payload: dict[str, Any] | None = None) -> bool:
    """Decide whether Chatterbox-Turbo should split lines into short chunks.

    - If the caller explicitly passes split_on_sentences True/False, honor it.
    - Otherwise (auto mode): chunk only if the available GPU has less than
      10 GB of VRAM or if no CUDA GPU is detected. This keeps 6GB cards like
      the RTX 2060 safe while avoiding unnecessary chunking on GPUs like the
      Runpod A5000, where model reload per chunk is far more expensive than
      rendering a slightly longer line.
    """
    payload = payload or {}
    explicit = payload.get("split_on_sentences")
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            # 10 GB threshold; adjust if experience shows a different sweet spot.
            return total_mem < 10 * 1024 * 1024 * 1024
    except Exception:
        pass
    return True
...
class JobManager:
    ...
    def _run_chatterbox_batch(self, job: Job, manifest_path: Path, batch_dir: Path, entries: list[dict[str, Any]], progress_fn: Any | None = None) -> int:
        ...
        If `progress_fn(completed, total)` is provided, it is called whenever
        the batch renderer emits a progress marker.
        """
        if not entries:
            return 0
        ...
        try:
            assert proc.stdout is not None
            progress_re = re.compile(r"^__CHATTERBOX_BATCH_PROGRESS__\s+(\d+)\s+(\d+)\s*$")
            for line in proc.stdout:
                self._append_log(job, line)
                m = progress_re.match(line.strip())
                if m and progress_fn:
                    try:
                        progress_fn(int(m.group(1)), int(m.group(2)))
                    except Exception:
                        pass
            proc.wait()
            self._append_log(job, f"\nProcess exited with return code: {proc.returncode}\n")
    ...
    def _run_single(self, job: Job, payload: dict[str, Any]) -> None:
        ...
        split_on_sentences = payload.get("split_on_sentences")
        if engine == "chatterbox" and split_on_sentences is None:
            split_on_sentences = should_chunk_chatterbox(payload)
        else:
            split_on_sentences = bool(split_on_sentences)
    ...
    def _run_batch_take(self, job: Job, payload: dict[str, Any]) -> None:
        ...
        take_chunk = payload.get("split_on_sentences")
        if engine == "chatterbox" and take_chunk is None:
            take_chunk = should_chunk_chatterbox(payload)
        else:
            take_chunk = bool(take_chunk)
        if engine == "chatterbox" and take_chunk and len(chunks) > 1:
    ...
    def _run_batch(self, job: Job, payload: dict[str, Any]) -> None:
        ...
        # Only batch-render Chatterbox if auto-detect says we don't need to chunk.
        # Chunking is still the safe default for low-VRAM GPUs like the RTX 2060.
        use_batch_chatterbox = not should_chunk_chatterbox(payload)
        if chatterbox_batch_entries and use_batch_chatterbox:
            def batch_progress(completed: int, total: int) -> None:
                self._set(
                    job,
                    result={
                        "kind": "tagged-script-batch",
                        "progress": progress_payload(min(display_total, completed), "rendering", {
                            "index": completed,
                            "role": "Chatterbox batch",
                            "engine": "chatterbox",
                            "words": 0,
                            "text": f"Rendering {completed}/{total} Chatterbox utterances",
                        }),
                        "manifest": str(manifest_path),
                        "batch_payload": payload_snapshot,
                    },
                )
            rc_batch = self._run_chatterbox_batch(job, manifest_path, batch_dir, chatterbox_batch_entries, progress_fn=batch_progress)
            if rc_batch != 0:
        ...
        for idx, item_meta in enumerate(lines, start=1):
            ...
            if engine == "chatterbox" and use_batch_chatterbox and str(output) in chatterbox_outputs:
